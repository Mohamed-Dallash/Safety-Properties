 ...
 */
 ...
 implicit class ExtendedFloat(val a: Float) extends AnyVal {
  ...
  object Operators {
	...
	private var P: Double = 0.01
	private var prev_P: Double = 0.01
	private var AccMax: Double = 5.4
	private var prev_AccMax: Double = 5.4
	private var AccBrakeMax: Double = 9.8
	private var prev_AccBrakeMax: Double = 9.8
	private var AccBrakeMin: Double = 2.9
	private var prev_AccBrakeMin: Double = 2.9
	private var vEgo: Double = 0.0
	private var vLeading: Double = 0.0
	private var distance: Double = 0.0
	private var t4: Double = 0.0
	private var t5: Double = 0.0
	private var t0: Double = 0.0
	private var distanceCheck: Boolean = false
	private var SD: Double = 0.0
	private var t3: Double = 0.0
	private var t2: Double = 0.0
	private var t1: Double = 0.0


	private def on_detectLeadingVehicle(vEgo: Double, vLeading: Double, distance: Double): Unit = {
		this.t0 = vEgo * P
		this.t1 = 0.5 * AccMax * P*P 
		this.t2 = vEgo + P * AccBrakeMax
		this.t3 = 2.0 * AccBrakeMin
		this.t4 = vLeading * vLeading
		this.t5 = 2.0 * AccBrakeMax
		this.SD = t0 + t1 + (t2 * t2)/t3 - t4/t5
		this.distanceCheck = distance >= SD 
	}

	private def detectLeadingVehicle_output(): Any = {
		List("checkSafeDistance", List(distanceCheck.toString))
	}
	...
	def evaluate(event_name: String, params: Any*): Option[Any] = {
		...
		event_name match {
			case "detectLeadingVehicle" => {
				if (params.length != 3) throw new IllegalArgumentException("Incorrect number of parameters for event detectLeadingVehicle")
				Try(params(0).toString.trim.toDouble) match {
					case Success(value) => this.vEgo = value
					case Failure(e) => println(s"Failed to convert to Double: $e")
				}
				Try(params(1).toString.trim.toDouble) match {
					case Success(value) => this.vLeading = value
					case Failure(e) => println(s"Failed to convert to Double: $e")
				}
				Try(params(2).toString.trim.toDouble) match {
					case Success(value) => this.distance = value
					case Failure(e) => println(s"Failed to convert to Double: $e")
				}
				on_detectLeadingVehicle(vEgo, vLeading, distance)
				event = detectLeadingVehicle_output()
			}
			case _ => event = List(event_name, params.toList)
			}
		prev_P = P
		prev_AccMax = AccMax
		prev_AccBrakeMax = AccBrakeMax
		prev_AccBrakeMin = AccBrakeMin
		Some(event)
	}
}
...
/*
  prop safeDistanceCheck : forall distanceCheck . checkSafeDistance(distanceCheck) -> distanceCheck = ''true'' 
*/

class Formula_safeDistanceCheck(monitor: Monitor) extends Formula(monitor) {
          
  override def evaluate(): Boolean = {
    // assignments1 (leaf nodes that are not rule calls):
      now(2) = build("checkSafeDistance")(V("distanceCheck"))
    ...
    // assignments5 (main formula excluding leaf nodes):
      now(3) = relationToConstant("distanceCheck",EQOP,"true").or(pre(3))
      now(1) = now(2).not().or(now(3))
      now(0) = var_distanceCheck.seen.imp(now(1)).forAll(var_distanceCheck.quantvar)
  ...
  val var_distanceCheck :: Nil = declareVariables(("distanceCheck",true))(0)

  varsInRelations = Set("distanceCheck")
  val indices: List[Int] = List()

  pre = Array.fill(4)(bddGenerator.False)
  now = Array.fill(4)(bddGenerator.False)

  txt = Array(
    "forall distanceCheck . checkSafeDistance(distanceCheck) -> distanceCheck = ''true''",
      "checkSafeDistance(distanceCheck) -> distanceCheck = ''true''",
      "checkSafeDistance(distanceCheck)",
      "distanceCheck = ''true''"
  )
...
class PropertyMonitor(preMonitor: PreMonitorTrait) extends Monitor(preMonitor) {
  ...
  def eventsInSpec: Set[String] = Set("checkSafeDistance")

  formulae ++= List(new Formula_safeDistanceCheck(this))
}
