 ...
 */
 ...
 implicit class ExtendedFloat(val a: Float) extends AnyVal {
  ...
  object Operators {
	...
	private def collide_output(): Any = {
		List("collide_rate", List(rate_check.toString))
	}
	...
	def evaluate(event_name: String, params: Any*): Option[Any] = {
		...
		prev_no_of_collisions = no_of_collisions
		Some(event)
	}
}
...
/*
  prop collisionRateCheck : forall rate_check . collide_rate(rate_check) -> rate_check = ''true'' 
*/

class Formula_collisionRateCheck(monitor: Monitor) extends Formula(monitor) {
          
  override def evaluate(): Boolean = {
    // assignments1 (leaf nodes that are not rule calls):
      now(2) = build("collide_rate")(V("rate_check"))
    ...
    // assignments5 (main formula excluding leaf nodes):
      now(3) = relationToConstant("rate_check",EQOP,"true").or(pre(3))
      now(1) = now(2).not().or(now(3))
      now(0) = var_rate_check.seen.imp(now(1)).forAll(var_rate_check.quantvar)
  ...
  val var_rate_check :: Nil = declareVariables(("rate_check",true))(0)

  varsInRelations = Set("rate_check")
  ...
  txt = Array(
    "forall rate_check . collide_rate(rate_check) -> rate_check = ''true''",
      "collide_rate(rate_check) -> rate_check = ''true''",
      "collide_rate(rate_check)",
      "rate_check = ''true''"
  )
...
class PropertyMonitor(preMonitor: PreMonitorTrait) extends Monitor(preMonitor) {
  ...
  def eventsInSpec: Set[String] = Set("collide_rate")

  formulae ++= List(new Formula_collisionRateCheck(this))
}
