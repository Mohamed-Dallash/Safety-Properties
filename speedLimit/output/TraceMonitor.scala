 ...
 */
 ...
 implicit class ExtendedFloat(val a: Float) extends AnyVal {
  ...
  object Operators {
	...
	private var vEgo: Int = 0
	private var roadSpeed: Int = 0


	private def on_SpeedChanged(vEgo: Int, roadSpeed: Int): Unit = {
	}

	private def SpeedChanged_output(): Any = {
		List("checkVehicleSpeedBelowLimit", List(vEgo.toString, roadSpeed.toString))
	}
	...
	def evaluate(event_name: String, params: Any*): Option[Any] = {
		...
		event_name match {
			case "SpeedChanged" => {
				if (params.length != 2) throw new IllegalArgumentException("Incorrect number of parameters for event SpeedChanged")
				...
				Try(params(1).toString.trim.toInt) match {
					case Success(value) => this.roadSpeed = value
					case Failure(e) => println(s"Failed to convert to Int: $e")
				}
				on_SpeedChanged(vEgo, roadSpeed)
				event = SpeedChanged_output()
			}
...
/*
  prop VehiclespeedCheck : forall vEgo . forall roadSpeed . checkVehicleSpeedBelowLimit(vEgo,roadSpeed) -> vEgo <= roadSpeed 
*/

class Formula_VehiclespeedCheck(monitor: Monitor) extends Formula(monitor) {
          
  override def evaluate(): Boolean = {
    // assignments1 (leaf nodes that are not rule calls):
      now(3) = build("checkVehicleSpeedBelowLimit")(V("vEgo"),V("roadSpeed"))
    ...
    // assignments5 (main formula excluding leaf nodes):
      now(4) = relation("vEgo",LEOP,"roadSpeed").or(pre(4))
      now(2) = now(3).not().or(now(4))
      now(1) = var_roadSpeed.seen.imp(now(2)).forAll(var_roadSpeed.quantvar)
      now(0) = var_vEgo.seen.imp(now(1)).forAll(var_vEgo.quantvar)
  ...
  val var_vEgo :: var_roadSpeed :: Nil = declareVariables(("vEgo",true), ("roadSpeed",true))(0)

  varsInRelations = Set("vEgo","roadSpeed")
  ...
  txt = Array(
    "forall vEgo . forall roadSpeed . checkVehicleSpeedBelowLimit(vEgo,roadSpeed) -> vEgo <= roadSpeed",
      "forall roadSpeed . checkVehicleSpeedBelowLimit(vEgo,roadSpeed) -> vEgo <= roadSpeed",
      "checkVehicleSpeedBelowLimit(vEgo,roadSpeed) -> vEgo <= roadSpeed",
      "checkVehicleSpeedBelowLimit(vEgo,roadSpeed)",
      "vEgo <= roadSpeed"
  )
...
class PropertyMonitor(preMonitor: PreMonitorTrait) extends Monitor(preMonitor) {
  ...
  def eventsInSpec: Set[String] = Set("checkVehicleSpeedBelowLimit")

  formulae ++= List(new Formula_VehiclespeedCheck(this))
}
